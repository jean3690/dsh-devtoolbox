...
function resolveConfig(config) {
	...
	if (!Array.isArray(agentTools) || agentTools.some((v) => typeof v !== "string")) throw new TypeError("dsh-devtoolbox: config.agentTools must be an array of strings");
	...
	for (const spec of userTools) {
		if (typeof spec?.name !== "string" || !/^[a-z][a-z0-9_]{1,31}$/.test(spec.name)) throw new Error(`dsh-devtoolbox: user tool name must match ^[a-z][a-z0-9_]{1,31}$, got ${JSON.stringify(spec?.name)}`);
		if (typeof spec?.run !== "string" || spec.run === "") throw new Error(`dsh-devtoolbox: user tool "${spec?.name}" needs a non-empty run expression`);
		if (typeof spec?.description !== "string" || spec.description === "") throw new Error(`dsh-devtoolbox: user tool "${spec?.name}" needs a description`);
	}
	...
	if (typeof saveDir !== "string" || saveDir === "") throw new TypeError("dsh-devtoolbox: config.saveDir must be a non-empty string");
	...
	run({ text, target, direction }) {
		...
		try {
			if (direction === "unescape") {
				...
				return {
					kind: "text",
					text: s.replace(/\\u([0-9a-fA-F]{4})/g, (_, h) => String.fromCharCode(parseInt(h, 16))).replace(/\\(['"\\bfnrtv0])/g, (m, c) => ({
						n: "\n",
						t: "	",
						r: "\r",
						b: "\b",
			...
			return {
				kind: "text",
				text: s.replace(/\\/g, "\\\\").replace(/'/g, "\\'").replace(/"/g, "\\\"").replace(/\n/g, "\\n").replace(/\r/g, "\\r").replace(/\t/g, "\\t").replace(/\x08/g, "\\b").replace(/\f/g, "\\f")
			};
		} catch {
			return {
				kind: "text",
				text: "Error: cannot parse input"
			};
		}
	}
};
...
const sort_lines = {
	...
	args: {
		text: {
			type: "string",
			required: true,
			description: "input text (one item per line)"
		},
		action: {
			type: "string",
			default: "sort",
			description: "sort | uniq | reverse | freq"
		},
		order: {
			type: "string",
			default: "asc",
		...
		limit: {
			type: "number",
			default: 10,
			description: "max rows for freq (1-1000)"
		}
	},
	run({ text, action, order, numeric, ignoreCase, limit }) {
		const lines = String(text).split(/\r\n|\r|\n/).map((l) => l.replace(/\r$/, ""));
		const desc = order === "desc";
		const cmp = numeric === true ? (a, b) => {
			const x = Number(a);
			const y = Number(b);
			return Number.isFinite(x) && Number.isFinite(y) ? x - y : a.localeCompare(b);
		} : (a, b) => ignoreCase === true ? a.toLowerCase().localeCompare(b.toLowerCase()) : a.localeCompare(b);
		switch (action) {
			case "uniq": {
				const seen = /* @__PURE__ */ new Set();
				const out = [];
			...
			case "freq": {
				...
				return {
					kind: "table",
					columns: ["value", "count"],
					rows: [...counts.entries()].sort((a, b) => b[1] - a[1]).slice(0, Math.max(1, Math.min(1e3, Math.floor(Number(limit)) || 10)))
				};
			}
			default: return {
				kind: "text",
				text: [...lines].sort((a, b) => (desc ? -1 : 1) * cmp(a, b)).join("\n")
			};
		}
	}
};
...
const data_url = {
	...
	args: {
		text: {
			type: "string",
			required: true,
			description: "text, or a data: URL to decode"
		},
		mime: {
			type: "string",
			description: "mime type for encoding (auto-detected by default)"
		},
		encoding: {
			type: "string",
			default: "base64",
			description: "base64 | plain"
		}
	},
	run({ text, mime, encoding }) {
		...
		if (/^data:/i.test(s)) {
			...
			if (m === null) return {
				kind: "text",
				text: "Error: malformed data URL"
			};
			const type = m[1] === void 0 || m[1] === "" ? "text/plain" : m[1];
		...
		const type = mime !== void 0 && String(mime) !== "" ? String(mime) : guessMime(s);
		if (encoding === "plain") return {
			kind: "text",
			text: `data:${type},${encodeURIComponent(s)}`
		};
		return {
			kind: "text",
			text: `data:${type};base64,${utf8ToBase64(s)}`
		};
	}
};
...
const qrcode = {
	...
	args: {
		...
		size: {
			type: "number",
			default: 256,
			description: "image size in px (16-1024)"
		},
		margin: {
			type: "number",
			default: 2,
	...
	async run({ text, size, margin, errorCorrection }) {
		const s = String(text);
		if (s === "") return {
			kind: "text",
			text: "Error: empty content"
		};
		const opts = {
			...
			errorCorrectionLevel: [
				...
				"Q",
				"H"
			].includes(String(errorCorrection).toUpperCase()) ? String(errorCorrection).toUpperCase() : "M"
		};
		try {
...
function jsonPathGet(root, path) {
	const p = path.trim();
	if (p === "") return {
		ok: false,
		error: "empty path"
	...
	while ((m = re.exec(body)) !== null) {
		if (m.index !== last) return {
			ok: false,
			error: `unexpected token near "${body.slice(last, m.index)}"`
		};
		last = re.lastIndex;
		if (m[1] !== void 0) parts.push({
			type: "key",
			key: m[1]
		});
		else if (m[2] !== void 0) parts.push({
			type: "index",
			index: Number(m[2])
		});
		else if (m[4] !== void 0) parts.push({
			type: "key",
			key: m[4]
		});
		else parts.push({ type: "wild" });
	}
	if (last !== body.length) return {
		ok: false,
		error: `unexpected token near "${body.slice(last)}"`
	};
	let cur = root;
	for (const part of parts) if (part.type === "wild") {
		if (Array.isArray(cur)) cur = cur.flatMap((v) => v);
		else if (cur !== null && typeof cur === "object") cur = Object.values(cur);
		else return {
			ok: false,
			error: "cannot wildcard a scalar"
		};
	} else if (part.type === "index") {
		if (!Array.isArray(cur)) return {
			ok: false,
			error: "indexing a non-array"
		};
		const i = part.index < 0 ? cur.length + part.index : part.index;
		if (i < 0 || i >= cur.length) return {
			ok: false,
			error: `index ${part.index} out of range`
		};
		cur = cur[i];
	} else {
		if (cur === null || typeof cur !== "object" || Array.isArray(cur)) return {
			ok: false,
			error: `cannot read key "${part.key}"`
		};
		if (!(part.key in cur)) return {
			ok: false,
			error: `key "${part.key}" not found`
		};
		cur = cur[part.key];
	}
	return {
...
const json_path = {
	...
	args: {
		...
		path: {
			type: "string",
			required: true,
			description: "JSONPath, e.g. $.items[0].name or $.*"
		}
	},
	run({ text, path }) {
		let doc;
...
const json_to_yaml = {
	...
	args: {
		text: {
			type: "string",
			required: true,
			description: "input JSON or YAML"
		},
		direction: {
			type: "string",
			default: "jsonToYaml",
			description: "jsonToYaml | yamlToJson"
		}
	},
	run({ text, direction }) {
		const s = String(text);
		try {
			if (direction === "yamlToJson") return {
				kind: "json",
				json: parse(s)
			};
...
async function aesEncrypt(text, password) {
	...
	const cipher = await crypto.subtle.encrypt({
		name: "AES-GCM",
		iv
	}, key, toArrayBuffer(enc.encode(text)));
...
function pemToArrayBuffer(pem) {
	...
	return {
		type: m[1] === "PUBLIC" ? "spki" : "pkcs8",
		data: toArrayBuffer(der)
	};
}
async function rsaEncrypt(text, pem) {
	const { type, data } = pemToArrayBuffer(pem);
	if (type !== "spki") throw new Error("RSA encryption needs a public key (BEGIN PUBLIC KEY)");
	const key = await crypto.subtle.importKey("spki", data, {
		name: "RSA-OAEP",
		hash: "SHA-256"
	}, false, ["encrypt"]);
...
async function rsaDecrypt(payload, pem) {
	const { type, data } = pemToArrayBuffer(pem);
	if (type !== "pkcs8") throw new Error("RSA decryption needs a private key (BEGIN PRIVATE KEY)");
	const key = await crypto.subtle.importKey("pkcs8", data, {
		name: "RSA-OAEP",
		hash: "SHA-256"
	}, false, ["decrypt"]);
...
const crypto_encrypt = {
	...
	args: {
		text: {
			type: "string",
			required: true,
			description: "plaintext to encrypt, or base64 ciphertext to decrypt"
		},
		mode: {
			type: "string",
			default: "encrypt",
		...
		algorithm: {
			type: "string",
			default: "aes",
			description: "aes (password) | rsa (PEM key)"
		},
		password: {
			type: "string",
			description: "AES password (PBKDF2-derived key)"
		},
		key: {
			type: "string",
			description: "PEM public key for RSA encryption / private key for decryption"
		}
	},
	async run({ text, mode, algorithm, password, key }) {
		const s = String(text);
		const algo = String(algorithm ?? "aes").toLowerCase();
		const isDecrypt = String(mode ?? "encrypt") === "decrypt";
		try {
			if (algo === "rsa") {
				if (key === void 0 || String(key) === "") return {
					kind: "text",
					text: "Error: RSA needs a PEM key (key=…)"
				};
				return {
					kind: "text",
					text: isDecrypt ? await rsaDecrypt(s, String(key)) : await rsaEncrypt(s, String(key))
				};
			}
			if (algo !== "aes") return {
				kind: "text",
				text: `Error: unsupported algorithm "${algo}" (aes | rsa)`
			};
			if (password === void 0 || String(password) === "") return {
				kind: "text",
				text: "Error: AES needs a password (password=…)"
			};
			return {
				kind: "text",
				text: isDecrypt ? await aesDecrypt(s, String(password)) : await aesEncrypt(s, String(password))
			};
		} catch (error) {
			return {
				kind: "text",
...
function parseIdCard(id) {
	...
	if (!(from15 ? /^\d{17}$/.test(s) : /^\d{17}[\dX]$/.test(s))) return {
		ok: false,
		error: "not a valid 15/18-digit ID number"
	};
	let sum = 0;
...
const id_card = {
	...
	args: { text: {
		type: "string",
		required: true,
		description: "15 or 18 digit ID number (one per line)"
	} },
	run({ text }) {
		...
		for (const line of lines) {
			...
			rows.push([
				...
				res.sex,
				res.region,
				res.from15 ? "from 15-digit" : "18-digit"
			]);
...
const UNITS = {
	...
	tb: {
		cat: "storage",
		toBase: (v) => v * 0xe8d4a51000,
		fromBase: (v) => v / 0xe8d4a51000
	},
	kib: {
		cat: "storage",
		toBase: (v) => v * 1024,
...
const unit_convert = {
	...
	args: {
		...
		from: {
			type: "string",
			required: true,
			description: "source unit, e.g. kb"
		},
		to: {
			type: "string",
			required: true,
			description: "target unit, e.g. mb"
		}
	},
	run({ value, from, to }) {
		...
		if (src === void 0 || dst === void 0) {
			...
			return {
				kind: "text",
				text: `Error: unknown unit "${src === void 0 ? f : t}" (known: ${known})`
			};
		}
		if (src.cat !== dst.cat) return {
			kind: "text",
			text: `Error: cannot convert ${src.cat} to ${dst.cat}`
		};
		const v = Number(String(value).trim().replace(/,/g, ""));
		if (!Number.isFinite(v)) return {
			kind: "text",
			text: "Error: value is not a number"
		};
...
const TZ_CACHE = /* @__PURE__ */ new Map();
...
function parseWallClock(input) {
	...
	if (fields[0] < 1970 || fields[1] < 1 || fields[1] > 12 || fields[2] < 1 || fields[2] > 31 || fields[3] > 23 || fields[4] > 59 || fields[5] > 59) return {
		ok: false,
		error: "fields out of range"
	};
	return {
		ok: true,
		fields
...
const time_convert = {
	...
	args: {
		text: {
			type: "string",
			required: true,
			description: "wall-clock time, e.g. 2026-08-19 10:00:00"
		},
		from: {
			type: "string",
	...
	run({ text, from, to }) {
		...
		try {
			...
			return {
				kind: "text",
				text: "Error: invalid IANA time zone"
			};
		}
		...
		const offStr = `${offMs < 0 ? "-" : "+"}${Math.abs(Math.floor(offMs / 36e5)).toString().padStart(2, "0")}:${String(Math.abs(Math.round(offMs % 36e5 / 6e4))).padStart(2, "0")}`;
		return {
			kind: "table",
			columns: [
				"zone",
...
const convertTools = Object.freeze([
	money,
	color,
	unit_convert,
	time_convert
...
const NO_BODY = /* @__PURE__ */ new Set(["GET", "HEAD"]);
const http_request = {
	id: "http_request",
	...
	args: {
		...
		headers: {
			type: "string",
			description: "extra headers: JSON object or \"Key: value\" lines"
		},
		body: {
			type: "string",
			description: "request body (not allowed for GET/HEAD)"
		},
		timeout: {
			type: "number",
			default: 1e4,
			description: "timeout in ms (1-300000)"
		},
		maxBytes: {
			type: "number",
			default: 512e3,
			description: "max response body bytes to keep"
		}
	},
	async run({ url, method, headers, body, timeout, maxBytes }) {
		...
		if (!METHODS.has(m)) return {
			kind: "text",
			text: `Error: unsupported method "${m}"`
		};
		if (NO_BODY.has(m) && String(body ?? "") !== "") return {
			kind: "text",
			text: `Error: ${m} requests cannot have a body`
		};
		const init = {
			method: m,
			headers: parseHeaders(headers === void 0 ? void 0 : String(headers))
		};
		if (!NO_BODY.has(m) && String(body ?? "") !== "") init.body = String(body);
		const ms = Math.max(1, Math.min(3e5, Math.floor(Number(timeout)) || 1e4));
		const limit = Math.max(1024, Math.min(10485760, Math.floor(Number(maxBytes)) || 512e3));
...
const apiTools = Object.freeze([http_request]);
//#endregion
//#region src/tools/network.ts
...
function parseCurlTokens(all) {
	...
	for (let i = 0; i < tokens.length; i++) {
		...
		switch (tok) {
			...
			case "-H":
			case "--header":
				if (tokens[i + 1] !== void 0) headers.push(tokens[++i]);
				break;
			case "-d":
			case "--data":
			case "--data-binary":
			case "--data-raw":
				if (tokens[i + 1] !== void 0) data.push(tokens[++i]);
				break;
			case "--json":
				jsonMode = true;
				if (tokens[i + 1] !== void 0) data.push(tokens[++i]);
			...
			case "-g":
			case "--globoff": break;
			default: if (tok.startsWith("--") && !tok.startsWith("--data") && !tok.startsWith("--request") && !tok.startsWith("--header") && !tok.startsWith("--user") && !tok.startsWith("--max")) {
...
const curl_parse = {
	...
	args: { text: {
		type: "string",
		required: true,
		description: "a curl command line"
	} },
	run({ text }) {
		const parsed = parseCurlTokens(tokenizeCommand(String(text)));
		if (parsed.url === "") return {
			kind: "text",
			text: "Error: no URL found in the curl command"
		};
		const headers = {};
...
function jwtSignatureBytes(token) {
...
async function hmacSha(alg, data, secret) {
	const hashName = {
		HS256: "SHA-256",
		HS384: "SHA-384",
		HS512: "SHA-512"
...
const jwt = {
	...
	args: {
		...
		secret: {
			type: "string",
			description: "HMAC secret to verify HS256/384/512 signatures"
		}
	},
	async run({ token, secret }) {
		...
		if (parts.length !== 3) return {
			kind: "text",
			text: "Error: not a JWT (expected header.payload.signature)"
		};
		const [header, payload, signature] = parts;
		const headerJson = base64UrlDecode(header);
		const payloadJson = base64UrlDecode(payload);
		if (headerJson === null || payloadJson === null) return {
			kind: "text",
			text: "Error: malformed base64 segment"
		};
		...
		if (parsedHeader === null || parsedPayload === null) return {
			kind: "text",
			text: "Error: header or payload is not valid JSON"
		};
		const alg = String(parsedHeader["alg"] ?? "");
		let verified = null;
		if (alg === "none") verified = signature === "";
		else if (secret !== void 0 && secret !== "") {
			if ({
				HS256: 32,
				HS384: 48,
				HS512: 64
			}[alg] !== void 0) {
				const expected = await hmacSha(alg, `${header}.${payload}`, String(secret));
			...
			} else if (alg.startsWith("RS") || alg.startsWith("ES")) verified = null;
		} else verified = null;
		...
		return {
			...
			json: {
				...
				notYet: nbf !== null && nbf > now,
				exp: exp === null ? null : (/* @__PURE__ */ new Date(exp * 1e3)).toISOString(),
				hint: verified === null && alg !== "none" ? "pass secret=… to verify an HS* signature (RS/ES need a public key, not supported here)" : void 0
			}
		};
	}
};
...
const hmac = {
	...
	async run({ text, key, algorithm, format }) {
		const alg = String(algorithm).toUpperCase();
		if (!HMAC_ALGS.includes(alg)) return {
			kind: "text",
			text: `Error: unsupported algorithm "${alg}" (use ${HMAC_ALGS.join(" / ")})`
		};
		try {
			const importKey = await crypto.subtle.importKey("raw", new TextEncoder().encode(String(key)), {
				name: "HMAC",
				hash: alg
			}, false, ["sign"]);
...
const networkTools = Object.freeze([
	curl_parse,
	url_parse,
	jwt,
	hmac
...
*/
async function hashFile(abs, algo) {
...
function fileHashTool(baseUrl) {
	return defineTool({
		...
		async execute(args) {
			...
			const { hex, bytes } = await hashFile(resolvePath(baseUrl, strArg(args.path, "")), algo);
			return {
				algorithm: algo,
				hex,
				bytes
...
*/
async function convertFileEncoding(abs, direction, target) {
	const data = await readFile(abs);
	let out;
...
function fileEncodeTool(baseUrl) {
	return defineTool({
		...
		async execute(args) {
			...
			return convertFileEncoding(abs, direction, args.output === void 0 ? abs : resolvePath(baseUrl, strArg(args.output, "")));
		}
	});
}
/**
...
var ToolboxService = class extends TypertRemoteService {
	...
	*/
	async save(request) {
		...
		if (target !== this.saveDir && !target.startsWith(root)) throw new Error(`dsh-devtoolbox: refused to write outside the save directory: ${target}`);

...
export function resolveConfig(config: Config | undefined): ResolvedConfig {
  ...
  if (!Array.isArray(agentTools) || agentTools.some(v => typeof v !== 'string')) {
    throw new TypeError('dsh-toolbox: config.agentTools must be an array of strings')
  }
  ...
  for (const spec of userTools) {
    if (typeof spec?.name !== 'string' || !/^[a-z][a-z0-9_]{1,31}$/.test(spec.name)) {
      throw new Error(`dsh-toolbox: user tool name must match ^[a-z][a-z0-9_]{1,31}$, got ${JSON.stringify(spec?.name)}`)
    }
    if (typeof spec?.run !== 'string' || spec.run === '') {
      throw new Error(`dsh-toolbox: user tool "${spec?.name}" needs a non-empty run expression`)
    }
    if (typeof spec?.description !== 'string' || spec.description === '') {
      throw new Error(`dsh-toolbox: user tool "${spec?.name}" needs a description`)
    }
  }
  ...
  if (typeof saveDir !== 'string' || saveDir === '') {
    throw new TypeError('dsh-toolbox: config.saveDir must be a non-empty string')
  }
